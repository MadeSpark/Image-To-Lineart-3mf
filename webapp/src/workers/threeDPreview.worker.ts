...
import { buildPreviewModelGltfBlob, buildSealPreviewModelGltfBlob } from '../utils/generator'

type PreviewModelArtworkPayload = Pick<
  ProcessedArtwork,
  'baseLoops' | 'lineLoops' | 'strokeLoops' | 'boardWidthMm' | 'boardHeightMm' | 'pixelsPerMm'
>
...
workerScope.onmessage = async (event: MessageEvent<BuildPreviewModelMessage>) => {
  const { requestId, artwork, baseplateSettings, extrudeSettings, sealSettings, workMode } = event.data
  ...
  try {
    const blob = workMode === 'seal' && sealSettings
      ? buildSealPreviewModelGltfBlob(artwork, baseplateSettings, sealSettings)
      : buildPreviewModelGltfBlob(artwork, baseplateSettings, extrudeSettings)
